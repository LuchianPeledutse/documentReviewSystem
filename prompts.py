...
def suggestions_based_on_technologies(text: str) -> str:
    ...
    Технологии: {text}
    Формат ответа: ответ должен быть в форме пронумерованного списка, который имеет не более 3 элементов
    Важно: предложения должны быть короткими и содержательными
    Примеры:
    ...
    ЧЕГО НЕ ДЕЛАТЬ: Писать что-то кроме списка
    """
    return the_prompt
